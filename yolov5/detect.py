...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.rectangle(im0, (0,0), (165, 50), (0, 0, 0), -1)

            class_name_count = 'Oyster-Closed'
            l = s[1:s.find(class_name_count)].split()[-1]
            if class_name_count in s:
                cv2.putText(im0,l + " " + class_name_count,(8,15), cv2.FONT_HERSHEY_COMPLEX, 0.5,(255,255,255),1,cv2.LINE_AA)

            class_name_count = 'Oyster-Open'
            l = s[1:s.find(class_name_count)].split()[-1]
            if class_name_count in s:
                cv2.putText(im0,l + " " + class_name_count,(8,40), cv2.FONT_HERSHEY_COMPLEX, 0.5,(255,255,255),1,cv2.LINE_AA)
